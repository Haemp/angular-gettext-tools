...
var Extractor = (function () {
    function Extractor(options) {
        this.options = _.extend({
            ...
            endDelim: '}}',
            markerName: 'gettext',
            jsAttr: [], /* Attributes to parse js from */
            extensions: {
                htm: 'html',
                html: 'html',
                php: 'html',
	...
	Extractor.prototype.extractHtml = function (filename, src) {
        ...
        $('*').each(function (index, n) {
            var node, plural, str, matches, jsString;
            node = $(n);
            ...
            for (var attr in node.attr()) {
                ...
                if (extractor.options.jsAttr.indexOf(attr) !== -1) {
                    jsString = node[0].attribs[attr];
                    extractor.extractJsInString(jsString, filename);
                }
    ...
    Extractor.prototype.extractJsInString = function (jsString, filename) {
        var extractor = this;
        var index = jsString.indexOf('gettext(\'');

        if (index !== -1) {
            var re = /gettext\(\'([^)]+)\'\)/g;
            var m = jsString.match(re);
            var str = '';
            for (var i = m.length - 1; i >= 0; i--) {
                str = m[i].replace('gettext(\'', '');
                str = str.replace('\')', '');
                extractor.addString(filename, str);	
            }
        }
